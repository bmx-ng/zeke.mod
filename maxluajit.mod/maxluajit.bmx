...
Function Invoke( L:Byte Ptr )
	...
	Local routine:TMember=TMember( lua_tolightobject( L,LUA_GLOBALSINDEX-2 ) )
	
	Local func:TFunction = TFunction(routine)
	Local meth:TMethod = TMethod(routine)
	...
	For Local i=0 Until args.length
		...
		Case LongTypeId
			args[i]=String.FromLong( lua_tonumber( L,i+1 ) )
		Case FloatTypeId
	...
	Select routine.TypeId()
		...
		Case LongTypeId
			lua_pushnumber L,t.ToString().ToLong()
		Case FloatTypeId
...
Function Index( L:Byte Ptr )
	...
	' is a routine : method or function ?
	Local routine:TMember = typeId.FindMethod( ident )
	If Not routine Then
		routine = typeId.FindFunction(ident)
	End If
	If routine Then
		lua_pushvalue L, 1
		lua_pushlightobject L, routine
	...
	If fld Then
		Select fld.TypeId() ' BaH - added more types
			...
			Case LongTypeId
				lua_pushnumber L,fld.GetLong( obj )
			Case FloatTypeId
	...
	If constant Then
		Select constant.TypeId() ' BaH - added more types
			Case IntTypeId, ShortTypeId, ByteTypeId
				lua_pushinteger(L, constant.GetInt())
			Case LongTypeId
				lua_pushnumber(L, constant.GetLong())
			Case FloatTypeId
				lua_pushnumber(L, constant.GetFloat())
			Case DoubleTypeId
	...
	If fld
		...
		Else
			...
			Select fld.TypeId()
				...
				Case LongTypeId
					fld.SetLong obj,lua_tonumber( L,3 )
				Case FloatTypeId
